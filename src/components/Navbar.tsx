        ...
        <div className="flex items-center justify-between h-16 md:h-[68px]">
          ...
          <div className="flex items-center space-x-2 sm:space-x-2.5">
            ...
            <button
              ...
              className={`flex items-center space-x-1.5 px-2.5 py-1.5 rounded-full text-xs font-medium border transition-all cursor-pointer shadow-sm ${
                user.role === 'teacher'
                  ? 'bg-[#10B981]/15 border-[#10B981]/40 text-[#10B981] hover:bg-[#10B981]/25'
                  : 'bg-[#141414] border-white/[0.08] hover:border-white/[0.2] text-[#A3A3A3] hover:text-white hover:bg-[#1C1C1C]'
              }`}
              title={`Peran saat ini: ${user.role === 'teacher' ? 'Guru / Pengajar' : 'Siswa'}. Klik untuk beralih mode simulasi.`}
            >
              {user.role === 'teacher' ? (
                <>
