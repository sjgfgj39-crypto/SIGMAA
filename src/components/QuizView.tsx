...
export const QuizView: React.FC<QuizViewProps> = ({
  ...
  useEffect(() => {
    setBookmarkedIds(SupabaseService.getBookmarks());
  ...
  const handleToggleBookmark = (qId: string) => {
    SupabaseService.toggleBookmark(qId);
    setBookmarkedIds(SupabaseService.getBookmarks());
  };
  ...
  const finalizeSubmission = (
    ...
    const attempt: QuizAttempt = {
      id: `attempt-${Date.now()}`,
      userId: SupabaseService.getUserProfile().id,
      moduleId: module.id,
    ...
    SupabaseService.recordQuizAttempt(attempt);
