...
export const DiscussionSection: React.FC<DiscussionSectionProps> = ({
  ...
  const [threads, setThreads] = useState<DiscussionThread[]>(SupabaseService.getDiscussions());
  ...
  const refreshThreads = () => {
    setThreads(SupabaseService.getDiscussions());
  };
  ...
  const handleCreateThread = (e: React.FormEvent) => {
    ...
    SupabaseService.createDiscussionThread({
  ...
  const handleAddReply = (threadId: string) => {
    ...
    SupabaseService.addReply(threadId, {
      threadId,
      authorId: user.id,
      authorName: user.name,
      authorRole: user.role,
  ...
  const handleToggleUpvote = (threadId: string, replyId?: string) => {
    SupabaseService.toggleUpvote(threadId, replyId);
    refreshThreads();
  };

  const handleTogglePin = (threadId: string) => {
    if (user.role !== 'teacher') return;
    SupabaseService.togglePinThread(threadId);
    refreshThreads();
  };
